...
// Functions
static void ImGui_ImplDX11_SetupRenderState(ImDrawData* draw_data, ComPtr<ID3D11DeviceContext> ctx)
{
...
void ImGui_ImplDX11_RenderDrawData(ImDrawData* draw_data)
{
    ...
    ImGui_ImplDX11_Data* bd = ImGui_ImplDX11_GetBackendData();
    ComPtr<ID3D11DeviceContext> ctx = bd->pd3dDeviceContext;
...
bool    ImGui_ImplDX11_Init(ComPtr<ID3D11Device> device, ComPtr<ID3D11DeviceContext> device_context)
{
    ImGuiIO& io = ImGui::GetIO();
    ...
    if (device.Get()->QueryInterface(IID_PPV_ARGS(&pDXGIDevice)) == S_OK)
    ...
    bd->pd3dDevice.Get()->AddRef();
    bd->pd3dDeviceContext.Get()->AddRef();
...
void ImGui_ImplDX11_Shutdown()
{
    ...
    if (bd->pd3dDevice)           { bd->pd3dDevice.Get()->Release(); }
    if (bd->pd3dDeviceContext)    { bd->pd3dDeviceContext.Get()->Release(); }
