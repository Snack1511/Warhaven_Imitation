...
CChargeTest* CChargeTest::Create()
{
	CChargeTest* pInstance = new CChargeTest();
...
HRESULT CChargeTest::Initialize()
{
	m_eAnimDivide = ANIM_DIVIDE::eBODYUPPER;

	m_eAnimType = ANIM_ATTACK;            // 애니메이션의 메쉬타입
	m_iAnimIndex = 14;                   // 현재 내가 사용하고 있는 애니메이션 순서(0 : IDLE, 1 : Run)
	m_eStateType = STATE_CHARGETEST;   // 나의 행동 타입(Init 이면 내가 시작할 타입)
	...
	m_iMaxChargeFrame = 30;
	m_eCurrentChargeKey = KEY::H;
	...
	m_eChargeAttackState = STATE_ATTACK_VERTICALCUT;
...
void CChargeTest::Enter(CUnit* pOwner, CAnimator* pAnimator, STATE_TYPE ePrevType, void* pData)
{
...
STATE_TYPE CChargeTest::Tick(CUnit* pOwner, CAnimator* pAnimator)
{
	return __super::Tick(pOwner, pAnimator);
}

void CChargeTest::Exit(CUnit* pOwner, CAnimator* pAnimator)
{
	/* 할거없음 */
...
STATE_TYPE CChargeTest::Check_Condition(CUnit* pOwner, CAnimator* pAnimator)
{
...
void CChargeTest::On_KeyFrameEvent(CUnit* pOwner, CAnimator* pAnimator, const KEYFRAME_EVENT& tKeyFrameEvent, _uint iSequence)
{
