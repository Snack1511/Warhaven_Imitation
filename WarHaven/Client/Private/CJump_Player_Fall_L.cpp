...
CJump_Player_Fall_L::CJump_Player_Fall_L()
{
}
...
CJump_Player_Fall_L* CJump_Player_Fall_L::Create()
{
    CJump_Player_Fall_L* pInstance = new CJump_Player_Fall_L();

    if (FAILED(pInstance->Initialize()))
    {
        Call_MsgBox(L"Failed to Initialize : CJump_Player_Fall_L");
...
HRESULT CJump_Player_Fall_L::Initialize()
{
    ...
    m_eStateType = STATE_JUMPFALL_PLAYER_L;   // 나의 행동 타입(Init 이면 내가 시작할 타입)
...
void CJump_Player_Fall_L::Exit(CUnit* pOwner, CAnimator* pAnimator)
{
    /* 할거없음 */
}

STATE_TYPE CJump_Player_Fall_L::Check_Condition(CUnit* pOwner, CAnimator* pAnimator)
{
    /* Player가 Walk로 오는 조건
    1. Jump 를 끝내면.
    */
