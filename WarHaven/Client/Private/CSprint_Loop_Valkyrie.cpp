...
CSprint_Loop_Valkyrie::CSprint_Loop_Valkyrie()
{
}
...
CSprint_Loop_Valkyrie* CSprint_Loop_Valkyrie::Create()
{
    CSprint_Loop_Valkyrie* pInstance = new CSprint_Loop_Valkyrie();

    if (FAILED(pInstance->Initialize()))
    {
        Call_MsgBox(L"Failed to Initialize : CSprint_Loop_Valkyrie");
        SAFE_DELETE(pInstance);
    }

    return pInstance;
}
HRESULT CSprint_Loop_Valkyrie::Initialize()
{
    m_eAnimType = ANIM_BASE_R;          // 애니메이션의 메쉬타입
    m_iAnimIndex = 48;                   // 현재 내가 사용하고 있는 애니메이션 순서(0 : IDLE, 1 : Run)
    m_eStateType = STATE_SPRINT_LOOP_VALKYRIE;   // 나의 행동 타입(Init 이면 내가 시작할 타입)
...
void CSprint_Loop_Valkyrie::Enter(CUnit* pOwner, CAnimator* pAnimator, STATE_TYPE ePrevType, void* pData )
{
    if (ePrevType == STATE_SPRINT_BEGIN_VALKYRIE || ePrevType == STATE_SPRINT_LOOP_VALKYRIE)
        m_fInterPolationTime = 0.f;
...
STATE_TYPE CSprint_Loop_Valkyrie::Tick(CUnit* pOwner, CAnimator* pAnimator)
{
    if (pOwner->Is_Air())
...
void CSprint_Loop_Valkyrie::Exit(CUnit* pOwner, CAnimator* pAnimator)
{
	CPhysics* pMyPhysicsCom = pOwner->Get_PhysicsCom();
...
STATE_TYPE CSprint_Loop_Valkyrie::Check_Condition(CUnit* pOwner, CAnimator* pAnimator)
{
