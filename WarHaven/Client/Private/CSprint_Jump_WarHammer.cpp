...
CSprint_Jump_WarHammer::CSprint_Jump_WarHammer()
{
}
...
CSprint_Jump_WarHammer* CSprint_Jump_WarHammer::Create()
{
    CSprint_Jump_WarHammer* pInstance = new CSprint_Jump_WarHammer();

    if (FAILED(pInstance->Initialize()))
    {
        Call_MsgBox(L"Failed to Initialize : CSprint_Jump_WarHammer");
        SAFE_DELETE(pInstance);
    }

    return pInstance;
}
HRESULT CSprint_Jump_WarHammer::Initialize()
{
    m_eAnimType = ANIM_BASE_R;          // 애니메이션의 메쉬타입
    m_iAnimIndex = 60;                   // 현재 내가 사용하고 있는 애니메이션 순서(0 : IDLE, 1 : Run)
    m_eStateType = STATE_SPRINT_JUMP_WARHAMMER;   // 나의 행동 타입(Init 이면 내가 시작할 타입)
...
void CSprint_Jump_WarHammer::Enter(CUnit* pOwner, CAnimator* pAnimator, STATE_TYPE ePrevType, void* pData )
{
...
STATE_TYPE CSprint_Jump_WarHammer::Tick(CUnit* pOwner, CAnimator* pAnimator)
{
	CTransform* pMyTransform = pOwner->Get_Transform();
	CPhysics* pMyPhysicsCom = pOwner->Get_PhysicsCom();
...
void CSprint_Jump_WarHammer::Exit(CUnit* pOwner, CAnimator* pAnimator)
{
    /* 할거없음 */
}

STATE_TYPE CSprint_Jump_WarHammer::Check_Condition(CUnit* pOwner, CAnimator* pAnimator)
{
