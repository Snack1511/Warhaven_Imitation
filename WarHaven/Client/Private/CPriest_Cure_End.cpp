...
CPriest_Cure_End* CPriest_Cure_End::Create()
{
    CPriest_Cure_End* pInstance = new CPriest_Cure_End();

    if (FAILED(pInstance->Initialize()))
    {
        Call_MsgBox(L"Failed to Initialize : CPriest_Cure_End");
        SAFE_DELETE(pInstance);
    }

    return pInstance;
}
HRESULT CPriest_Cure_End::Initialize()
{
	m_eAnimDivide = ANIM_DIVIDE::eBODYUPPER;

    m_eAnimType = ANIM_ATTACK;            // 애니메이션의 메쉬타입
    m_iAnimIndex = 7;                   // 현재 내가 사용하고 있는 애니메이션 순서(0 : IDLE, 1 : Run)
    m_eStateType = STATE_CURE_END_PRIEST;   // 나의 행동 타입(Init 이면 내가 시작할 타입)
...
void CPriest_Cure_End::Enter(CUnit* pOwner, CAnimator* pAnimator, STATE_TYPE ePrevType, void* pData )
{
    /* Owner의 Animator Set Idle로 */
    __super::Enter(pOwner, pAnimator, ePrevType, pData);
}

STATE_TYPE CPriest_Cure_End::Tick(CUnit* pOwner, CAnimator* pAnimator)
{
    return __super::Tick(pOwner, pAnimator);
}

void CPriest_Cure_End::Exit(CUnit* pOwner, CAnimator* pAnimator)
{
	__super::Exit(pOwner, pAnimator);
}

STATE_TYPE CPriest_Cure_End::Check_Condition(CUnit* pOwner, CAnimator* pAnimator)
{
    if (KEY(RBUTTON, NONE))
        return m_eStateType;
...
void CPriest_Cure_End::On_KeyFrameEvent(CUnit* pOwner, CAnimator* pAnimator, const KEYFRAME_EVENT& tKeyFrameEvent, _uint iSequence)
{
