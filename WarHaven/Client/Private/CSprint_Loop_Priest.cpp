...
CSprint_Loop_Priest::CSprint_Loop_Priest()
{
}
...
CSprint_Loop_Priest* CSprint_Loop_Priest::Create()
{
    CSprint_Loop_Priest* pInstance = new CSprint_Loop_Priest();

    if (FAILED(pInstance->Initialize()))
    {
        Call_MsgBox(L"Failed to Initialize : CSprint_Loop_Priest");
        SAFE_DELETE(pInstance);
    }

    return pInstance;
}
HRESULT CSprint_Loop_Priest::Initialize()
{
    m_eAnimType = ANIM_BASE_R;          // 애니메이션의 메쉬타입
    m_iAnimIndex = 47;                   // 현재 내가 사용하고 있는 애니메이션 순서(0 : IDLE, 1 : Run)
    m_eStateType = STATE_SPRINT_LOOP_PRIEST;   // 나의 행동 타입(Init 이면 내가 시작할 타입)
    ...
    m_vecAdjState.push_back(STATE_SPRINTATTACK_BEGIN_PRIEST);

    return S_OK;
}

void CSprint_Loop_Priest::Enter(CUnit* pOwner, CAnimator* pAnimator, STATE_TYPE ePrevType, void* pData )
{
    if (ePrevType == STATE_SPRINT_BEGIN_PRIEST || ePrevType == STATE_SPRINT_LOOP_PRIEST)
        m_fInterPolationTime = 0.f;
...
STATE_TYPE CSprint_Loop_Priest::Tick(CUnit* pOwner, CAnimator* pAnimator)
{
    if (pOwner->Is_Air())
...
void CSprint_Loop_Priest::Exit(CUnit* pOwner, CAnimator* pAnimator)
{
	CPhysics* pMyPhysicsCom = pOwner->Get_PhysicsCom();
...
STATE_TYPE CSprint_Loop_Priest::Check_Condition(CUnit* pOwner, CAnimator* pAnimator)
{
