...
wstring CUI_MainPlay::m_wstrModeText = TEXT("모드 - 훈련소");
...
void CUI_MainPlay::Create_StageBtn()
{
	...
	for (int i = 0; i < 4; ++i)
	{
		...
		if (i == 0)
		{
			...
			m_pStageSelectBtn[i]->Set_FontText(TEXT("점령전"));
		...
		if (i == 1)
		{
			m_pStageSelectBtn[i]->Set_FontOffset(vFontOffset.x, vFontOffset.y);
			m_pStageSelectBtn[i]->Set_FontScale(m_fFontSize);
			m_pStageSelectBtn[i]->Set_FontText(TEXT("진격전"));
