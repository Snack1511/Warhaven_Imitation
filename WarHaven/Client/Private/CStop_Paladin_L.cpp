...
CStop_Paladin_L::CStop_Paladin_L()
{
}
...
CStop_Paladin_L* CStop_Paladin_L::Create()
{
	CStop_Paladin_L* pInstance = new CStop_Paladin_L();

	if (FAILED(pInstance->Initialize()))
	{
		Call_MsgBox(L"Failed to Initialize : CStop_Paladin_L");
		SAFE_DELETE(pInstance);
	}

	return pInstance;
}
HRESULT CStop_Paladin_L::Initialize()
{
	...
	m_eStateType = STATE_STOP_PALADIN_L;   // 나의 행동 타입(Init 이면 내가 시작할 타입)
...
void CStop_Paladin_L::Enter(CUnit* pOwner, CAnimator* pAnimator, STATE_TYPE ePrevType, void* pData)
{
...
STATE_TYPE CStop_Paladin_L::Tick(CUnit* pOwner, CAnimator* pAnimator)
{
	//if (KEY(W, HOLD) || KEY(A, HOLD) || KEY(S, HOLD) || KEY(D, HOLD))
		//return STATE_RUN_BEGIN_PALADIN_L;
	return __super::Tick(pOwner, pAnimator);
}

void CStop_Paladin_L::Exit(CUnit* pOwner, CAnimator* pAnimator)
{
...
STATE_TYPE CStop_Paladin_L::Check_Condition(CUnit* pOwner, CAnimator* pAnimator)
{
	return __super::Check_Condition(pOwner, pAnimator);
}

void CStop_Paladin_L::On_KeyFrameEvent(CUnit* pOwner, CAnimator* pAnimator, const KEYFRAME_EVENT& tKeyFrameEvent, _uint iSequence)
{
