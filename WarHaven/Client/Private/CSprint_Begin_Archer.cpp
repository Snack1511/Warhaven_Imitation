...
CSprint_Begin_Archer::CSprint_Begin_Archer()
{
}
...
CSprint_Begin_Archer* CSprint_Begin_Archer::Create()
{
	CSprint_Begin_Archer* pInstance = new CSprint_Begin_Archer();

	if (FAILED(pInstance->Initialize()))
	{
		Call_MsgBox(L"Failed to Initialize : CSprint_Begin_Archer");
		SAFE_DELETE(pInstance);
	}

	return pInstance;
}
HRESULT CSprint_Begin_Archer::Initialize()
{
...
void CSprint_Begin_Archer::Enter(CUnit* pOwner, CAnimator* pAnimator, STATE_TYPE ePrevType, void* pData)
{
...
STATE_TYPE CSprint_Begin_Archer::Tick(CUnit* pOwner, CAnimator* pAnimator)
{
	if (pOwner->Is_Air())
...
void CSprint_Begin_Archer::Exit(CUnit* pOwner, CAnimator* pAnimator)
{
	CPhysics* pMyPhysicsCom = pOwner->Get_PhysicsCom();
...
STATE_TYPE CSprint_Begin_Archer::Check_Condition(CUnit* pOwner, CAnimator* pAnimator)
{
...
void CSprint_Begin_Archer::On_KeyFrameEvent(CUnit* pOwner, CAnimator* pAnimator, const KEYFRAME_EVENT& tKeyFrameEvent, _uint iSequence)
{
	switch (iSequence)
	{
	case 0:
