...
CHit_Groggy_Valkyrie::CHit_Groggy_Valkyrie()
{
}
...
CHit_Groggy_Valkyrie* CHit_Groggy_Valkyrie::Create()
{
    CHit_Groggy_Valkyrie* pInstance = new CHit_Groggy_Valkyrie();

    if (FAILED(pInstance->Initialize()))
    {
        Call_MsgBox(L"Failed to Initialize : CHit_Groggy_Valkyrie");
        SAFE_DELETE(pInstance);
    }

    return pInstance;
}
HRESULT CHit_Groggy_Valkyrie::Initialize()
{
    
    m_eAnimType = ANIM_HIT;            // 애니메이션의 메쉬타입
    m_iAnimIndex = 9;                   // 현재 내가 사용하고 있는 애니메이션 순서(0 : IDLE, 1 : Run)
    m_eStateType = STATE_GROGGYHIT_VALKYRIE;   // 나의 행동 타입(Init 이면 내가 시작할 타입)
...
void CHit_Groggy_Valkyrie::Enter(CUnit* pOwner, CAnimator* pAnimator, STATE_TYPE ePrevType, void* pData )
{
...
void CHit_Groggy_Valkyrie::Exit(CUnit* pOwner, CAnimator* pAnimator)
{
	//pOwner->Enable_UnitCollider(CUnit::WEAPON_R, false);

}

STATE_TYPE CHit_Groggy_Valkyrie::Check_Condition(CUnit* pOwner, CAnimator* pAnimator)
{
